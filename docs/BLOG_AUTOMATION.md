...
2. Calculate the current Monday date in America/Detroit as `week_start`. Read existing post slugs, topics, keywords and publication slots. Check the weekly slot using `publication_kind='weekly'`; launch articles do not occupy it. If that weekly slot is already published, verify its URL and finish without a duplicate. Never delete or overwrite an existing post to make room.
...
10. Execute the prepared SQL through Supabase `execute_sql`. The transaction rechecks exact cover eligibility, inserts at most one weekly post per `week_start` using a partial unique index, and records the aggregate snapshot and photo provenance privately. The weekly unique index and unique article slug prevent duplicate publication on retries. Private run keys are `weekly:<Monday>` for scheduled posts and `launch:<slug>` for launch posts. If no row is inserted, check whether the slot already exists or the cover became ineligible. Never report success solely because SQL returned no error.
...
The checked-in blog migrations are already applied to production; their version numbers match the remote migration ledger. This website repository contains only the blog additions, not the app database's full migration history. Do not run a standalone database reset or blindly push migrations from this partial history.

## Launch collection

On September 15, 2026, the user requested at least one article in every category. Six additional articles use `publication_kind='launch'` with the actual week `2026-09-14`; the original Nutrition article remains that week's weekly publication. Each launch article has its own slug and private `launch:<slug>` run key. No future weekly slots are reserved, and no dates are backfilled to bypass the publishing limit. The normal automation continues with one `weekly` article per Monday, next on September 21, 2026.

| Category    | Launch article                       | Primary keyword                                |
| ----------- | ------------------------------------ | ---------------------------------------------- |
| Weight loss | calorie-deficit-without-overthinking | calorie deficit without counting every calorie |
| Weight gain | healthy-weight-gain-small-appetite   | how to gain weight with a small appetite       |
| Muscle      | building-muscle-basics               | building muscle for beginners                  |
| Workouts    | beginner-strength-training-two-days  | beginner strength training routine             |
| Recipes     | greek-yogurt-breakfast-bowl          | Greek yogurt breakfast bowl                    |
| Community   | what-your-food-log-can-tell-you      | how to use a food log                          |

These are editorial keyword targets, not measured search-volume claims. Future posts should add distinct reader questions rather than repeat the launch introductions.
